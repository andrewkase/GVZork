...
std::vector<std::string> Input::split(int i) {
    ...
    for (int j = 0; j < command_input.length(); j++) {
        if (command_input[j] != ' ') {
            if (word_count == 0) {
                first += command_input[j];
            }
            command_input_2 += command_input[j];
        }
        
        if (command_input[j] == ' ' || j == command_input.length() - 1) {
            if (word_count != 0) {
    ...
    if (i == 1) {
        return {first};  // Return only the first word if `i == 1`
    } else if (i == 2) {
        return rest_of_input;  // Return the rest of the words if `i == 2`
    }
